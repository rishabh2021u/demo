...
def uplod():
    ...
    subprocess.Popen("ffmpeg -i DASH_480.mp4 -i DASH_audio.mp4 -c:v copy -c:a aac output1.mp4 -y", shell=True, stdout=subprocess.PIPE)
    subprocess.Popen("ffmpeg -i output1.mp4 -vf 'pad=height=ih+30:x=0:y=0:color=black, drawtext=fontfile=/path/to/font.ttf:text='R_Chodi_is_now_bakchodi.org':fontcolor=white:fontsize=24:x=(w-text_w)/2:y=h-th,drawtext=:text='bakchodi.org':fontcolor=#e7e7e7:fontsize=10:x=1:y=(h-text_h)/2:box=1:boxcolor=black@0.5:boxborderw=3:x=w-text_w:y=(h-text_h)/2' -codec:a copy " + id + "video.mp4 -y", shell=True, stdout=subprocess.PIPE)
    subprocess.Popen("./rclone copy " + id + "video.mp4 " + "onedrive:public --config rclone.conf --ignore-existing && rm " + id + "video.mp4", shell=True, stdout=subprocess.PIPE)
